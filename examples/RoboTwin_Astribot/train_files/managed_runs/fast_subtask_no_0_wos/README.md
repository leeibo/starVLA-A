...
`examples/RoboTwin_Astribot/train_files/managed_runs/fast_subtask_no_0_wos/logs/`.
...
This run starts from the configured base VLM/action-token checkpoint and does
not load a history-trained `trainer.pretrained_checkpoint` by default.
...
`results/Checkpoints/fast_subtask_no_0_wos` unless `POLICY_CKPT_PATH` is set.
Default port is `7980`.

Outputs are written to `results/Checkpoints/fast_subtask_no_0_wos`.
