...
- history length: 12 history frames (`history.max_frames: 12`)
- state input: enabled (`include_state: true`)
- state encoder: one state token per input frame, 18-D state projected to the VLM hidden size

Launch:

```bash
bash examples/RoboTwin_Astribot/train_files/managed_runs/fast_subtask_action_12_ws/run_train.sh
```

Launch from the run directory with local GPU/log handling:

```bash
cd examples/RoboTwin_Astribot/train_files/managed_runs/fast_subtask_action_12_ws
GPU_IDS=0,1 ./start_train.sh
```

Submit through `yhbatch`:

```bash
yhbatch examples/RoboTwin_Astribot/train_files/managed_runs/fast_subtask_action_12_ws/submit_yhbatch.sh
```
...
`examples/RoboTwin_Astribot/train_files/managed_runs/fast_subtask_action_12_ws/logs/`.
...
The FAST tokenizer is loaded from
`./playground/Pretrained_models/fast`, so batch jobs do not need Hugging Face
network access for `physical-intelligence/fast`.
...
```bash
bash examples/RoboTwin_Astribot/train_files/managed_runs/fast_subtask_action_12_ws/run_policy_server.sh
```

The server auto-selects the latest checkpoint under
`results/Checkpoints/fast_subtask_action_12_ws` unless `POLICY_CKPT_PATH` is
set. Default port is `7980`. Clients must send `state_history` aligned with the
input image list for this `ws` run.

Outputs are written to `results/Checkpoints/fast_subtask_action_12_ws`.
