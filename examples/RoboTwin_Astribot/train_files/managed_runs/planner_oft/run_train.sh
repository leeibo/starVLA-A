...
STAGE="${PLANNER_OFT_STAGE:-vla}"
case "${STAGE}" in
  planner)
    CONFIG_YAML="${SCRIPT_DIR}/planner_config.yaml"
    TRAIN_ENTRYPOINT="starVLA/training/train_planner_oft_vlm.py"
    ;;
  vla)
    CONFIG_YAML="${SCRIPT_DIR}/config.yaml"
    TRAIN_ENTRYPOINT="starVLA/training/train_planner_oft_vla.py"
    ;;
  *)
    echo "Invalid PLANNER_OFT_STAGE=${STAGE}. Expected planner or vla." >&2
    exit 2
    ;;
esac
...
MAIN_PROCESS_PORT="${MAIN_PROCESS_PORT:-${MASTER_PORT:-29501}}"
