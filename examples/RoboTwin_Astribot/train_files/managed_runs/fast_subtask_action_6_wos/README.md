# fast_subtask_action_6_wos

This managed run trains `QwenFast` on Astribot LeRobot data with:
...
- initialization checkpoint: `results/Checkpoints/fast_subtask_action_12_wos/checkpoints/steps_100000_pytorch_model.pt`
...
`examples/RoboTwin_Astribot/train_files/managed_runs/fast_subtask_action_6_wos/logs/`.
...
This run uses `trainer.pretrained_checkpoint` to load model weights from
`fast_subtask_action_12_wos`, then starts a new optimizer and scheduler because
...
`results/Checkpoints/fast_subtask_action_6_wos` unless `POLICY_CKPT_PATH` is
set. Default port is `7980`.

Outputs are written to `results/Checkpoints/fast_subtask_action_6_wos`.
