...
`examples/RoboTwin_Astribot/train_files/managed_runs/fast_subtask_action_6_ws/logs/`.
...
`results/Checkpoints/fast_subtask_action_6_ws` unless `POLICY_CKPT_PATH` is set.
Default port is `7980`. Clients must send `state_history` aligned with the
input image list for this `ws` run.

Outputs are written to `results/Checkpoints/fast_subtask_action_6_ws`.
