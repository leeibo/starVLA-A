...
def add_new_tokens(
    ...
    Returns:
      ...
      - action_token_start_idx: minimum token id among the requested target tokens
      - action_token_end_idx: maximum token id among the requested target tokens
    Notes:
    ...
    # Qwen3 tokenizers may assign added tokens into reserved embedding rows
    # that already exist in the model, so initialization must be based on the
    # actual token ids instead of only the appended embedding range.
    added_now = 0
    if to_add_tokens:
        if as_special:
            try:
                added_now = tokenizer.add_special_tokens(
                    {"additional_special_tokens": to_add_tokens},
                ...
                merged_special_tokens = existing_special_tokens + [
                    token for token in to_add_tokens if token not in existing_special_tokens
                ]
                before_len = len(tokenizer)
                tokenizer.add_special_tokens({"additional_special_tokens": merged_special_tokens})
                added_now = len(tokenizer) - before_len
        else:
            added_now = tokenizer.add_tokens(to_add_tokens)

    # 4) Build mapping first, then resize to cover the actual token ids.
    mapping = {t: int(tokenizer.convert_tokens_to_ids(t)) for t in new_tokens}
    token_ids_to_init = sorted({mapping[t] for t in to_add_tokens})
    target_size = max(old_embed_size, len(tokenizer), max(mapping.values()) + 1)
    ...
    # 6) Initialize exactly the rows used by tokens added in this run.
    if token_ids_to_init:
        embedding_modules = [model.get_input_embeddings()]
        output_embed = model.get_output_embeddings()
        if output_embed is not None and output_embed is not embedding_modules[0]:
            embedding_modules.append(output_embed)

        with torch.no_grad():
            for embed in embedding_modules:
                row_ids = [idx for idx in token_ids_to_init if idx < embed.weight.shape[0]]
                if not row_ids:
                    continue
                idx = torch.tensor(row_ids, device=embed.weight.device, dtype=torch.long)
                if init_strategy == "avg":
                    ref_vec = old_embed.weight.mean(dim=0).to(device=embed.weight.device, dtype=embed.weight.dtype)
                    embed.weight[idx] = ref_vec.expand(len(row_ids), -1)
                elif init_strategy == "zero":
                    embed.weight[idx].zero_()
                elif init_strategy == "normal":
                    values = torch.empty(
                        (len(row_ids), embed.weight.shape[1]),
                        device=embed.weight.device,
                        dtype=embed.weight.dtype,
                    )
                    nn.init.normal_(values, mean=0.0, std=0.02)
                    embed.weight[idx] = values
                else:
                    raise ValueError(f"Unknown init_strategy: {init_strategy}")

    action_token_start_idx = min(mapping.values())
    action_token_end_idx = max(mapping.values())
...
def main():
    parser = argparse.ArgumentParser(description="Add special tokens to a Qwen-VL model and save to local directory.")
    ...
    parser.add_argument(
        ...
        help="Attention implementation used while loading the source model",
    )
    ...
    suffix = "" if len(tokens) <= 3 else f", ... {tokens[-1]}"
    print(f"[INFO] Tokens to process: {len(tokens)} ({preview}{suffix})")
    ...
    elif args.device not in {"cpu", "mps"}:
        raise ValueError(f"Unsupported --device {args.device!r}; expected cuda / cpu / mps / auto")
