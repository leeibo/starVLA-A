...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._astribot_subtask_by_episode = self._load_astribot_subtask_metadata()
    ...
    def _get_lerobot_modality_meta(self) -> LeRobotModalityMetadata:
        ...
        return self._infer_lerobot_modality_meta_from_info()

    def _infer_lerobot_modality_meta_from_info(self) -> LeRobotModalityMetadata:
        """Infer modality metadata for LeRobot v2.1 datasets without modality.json."""
        info_meta = self._get_lerobot_info_meta()
        features = info_meta.get("features", {})

        def _feature_dim(original_key: str) -> int:
        ...
        def _find_feature_key(candidates: list[str], *, dtype: str | None = None) -> str:
            for candidate in candidates:
                if candidate in features:
                    return candidate
            for key, meta in features.items():
                if dtype is not None and meta.get("dtype") != dtype:
                    continue
                for candidate in candidates:
                    if key.endswith(candidate):
                        return key
            raise KeyError(
                f"Unable to infer feature key from {candidates}. "
                f"Available features: {list(features)}"
            )

        state_original = _find_feature_key(["observation.state"])
        action_original = _find_feature_key(["action"])
        state_dim = _feature_dim(state_original)
        action_dim = _feature_dim(action_original)

        state_meta = {}
        state_config = self.modality_configs.get("state")
        state_keys = state_config.modality_keys if state_config is not None else []
        for key in state_keys:
            subkey = key.replace("state.", "", 1)
            state_meta[subkey] = {
                "original_key": state_original,
                "start": 0,
                "end": state_dim,
                "absolute": True,
                "dtype": features[state_original].get("dtype", "float32"),
            }

        action_meta = {}
        action_config = self.modality_configs.get("action")
        action_keys = action_config.modality_keys if action_config is not None else []
        for key in action_keys:
            subkey = key.replace("action.", "", 1)
            action_meta[subkey] = {
                "original_key": action_original,
                "start": 0,
                "end": action_dim,
                "absolute": True,
                "dtype": features[action_original].get("dtype", "float32"),
            }

        image_features = [key for key, meta in features.items() if meta.get("dtype") == "image"]
        video_meta = {}
        video_config = self.modality_configs.get("video")
        video_keys = video_config.modality_keys if video_config is not None else []
        for key in video_keys:
            subkey = key.replace("video.", "", 1)
            candidates = [
                f"observation.images.{subkey}",
                f"observation.image.{subkey}",
                subkey,
            ]
            try:
                original_key = _find_feature_key(candidates, dtype="image")
            except KeyError:
                if len(image_features) != 1:
                    raise
                original_key = image_features[0]
            video_meta[subkey] = {"original_key": original_key}

        annotation_meta = None
        language_config = self.modality_configs.get("language")
        if language_config is not None and language_config.modality_keys and "task_index" in features:
            annotation_meta = {}
            for key in language_config.modality_keys:
                subkey = key.replace("annotation.", "", 1)
                annotation_meta[subkey] = {"original_key": "task_index"}
    ...
    def _load_astribot_subtask_metadata(self) -> dict[int, dict]:
        metadata_path = self.dataset_path / "meta" / "astribot_subtask_metadata.json"
        ...
        for episode in episodes:
            if not isinstance(episode, dict):
                continue
            episode_index = episode.get("lerobot_episode_index", episode.get("episode_index"))
            if episode_index is None:
                continue
            by_episode[int(episode_index)] = episode
        return by_episode
    ...
    def _history_enabled(self) -> bool:
        value = self._history_cfg_get("enabled", False)
    ...
    def _max_history_frames(self) -> int | None:
        value = self._history_cfg_get("max_history_frames", None)
        if value is None:
            value = self._history_cfg_get("max_frames", None)
    ...
    def _history_stride(self) -> int:
        value = self._history_cfg_get("stride", None)
        if value is None:
            action_keys = self.modality_keys.get("action", [])
            if action_keys:
                value = len(self.delta_indices[action_keys[0]])
            else:
                value = 1
        stride = int(value)
        if stride <= 0:
            raise ValueError(f"history.stride must be positive, got {stride}")
        return stride

    def _normalize_video_key(self, key: str | None) -> str | None:
        if key is None:
            return None
        key = str(key)
        if key and not key.startswith("video."):
            key = f"video.{key}"
        return key

    def _resolve_history_video_key(self) -> str:
        configured_key = self._normalize_video_key(
            self._history_cfg_get("camera_key", self._history_cfg_get("video_key", None))
        )
        candidates = [
            configured_key,
            "video.camera_head",
        ...
        for key in candidates:
            if key in video_keys:
                return key
        if configured_key:
            raise ValueError(f"Configured history camera key {configured_key!r} not found in video keys: {video_keys}")
        if not video_keys:
            raise ValueError("history.enabled=True but dataset has no video modality keys")
        return video_keys[0]

    def _find_trajectory_column(self, configured_column, aliases: list[str]) -> str:
        assert self.curr_traj_data is not None, "No trajectory data loaded"
        candidates = []
        if configured_column:
            candidates.append(str(configured_column))
        candidates.extend(aliases)
        columns = list(self.curr_traj_data.columns)
        for column in candidates:
            if column in self.curr_traj_data.columns:
                return column
        for column in columns:
            suffix = str(column).split(".")[-1]
            if suffix in candidates:
                return column
        raise KeyError(f"None of columns {candidates} found. Available columns: {columns}")

    def _is_keyframe_value(self, value) -> bool:
        if isinstance(value, str):
            return value.lower() in {"1", "true", "yes", "y"}
        try:
            ...
            if arr.size == 0:
                return False
            return bool(float(arr[0]) == 1.0)
        except Exception:
            return bool(value)

    def _keyframe_history_indices(self, column: str, base_index: int, max_history: int | None) -> list[int]:
        assert self.curr_traj_data is not None, "No trajectory data loaded"
        values = self.curr_traj_data[column].to_numpy()
        end = min(base_index, len(values))
        history_indices = [i for i in range(end) if self._is_keyframe_value(values[i])]
        if max_history is not None:
            history_indices = history_indices[-max_history:]
        return history_indices + [base_index]
    ...
    def _history_frame_indices(self, base_index: int) -> list[int]:
        ...
        if mode == "action_keyframe":
            stride = self._history_stride()
            history_indices = []
            step = base_index - stride
            while step >= 0:
                history_indices.append(step)
                if max_history is not None and len(history_indices) >= max_history:
                    break
                step -= stride
            return list(reversed(history_indices)) + [base_index]
        if mode == "motion_keyframe":
            column = self._find_trajectory_column(
                self._history_cfg_get("motion_keyframe_column", None),
                ["motion_keyframe", "is_motion_keyframe"],
            )
            return self._keyframe_history_indices(column, base_index, max_history)
        if mode == "subtask_keyframe":
            column = self._find_trajectory_column(
            ...
            return self._keyframe_history_indices(column, base_index, max_history)
        raise ValueError(
            f"Invalid history.mode={mode!r}; expected action_keyframe, motion_keyframe, or subtask_keyframe"
        )

    def _image_resize_size(self) -> tuple[int, int]:
        size = self._data_cfg_get("obs_image_size", [224, 224])
        if isinstance(size, (list, tuple)) and len(size) == 2:
            return int(size[1]), int(size[0])
        return 224, 224

    def _pack_history_images(self, trajectory_id: int, base_index: int) -> tuple[list[Image.Image], list[int]]:
        video_key = self._resolve_history_video_key()
        frame_indices = self._history_frame_indices(base_index)
        frames = self.get_video(
            trajectory_id,
            video_key,
            base_index,
            step_indices=np.asarray(frame_indices, dtype=int),
        )
        resize_size = self._image_resize_size()
        images = [Image.fromarray(frame).resize(resize_size) for frame in frames]
        return images, frame_indices

    def _pack_history_state(self, trajectory_id: int, base_index: int, frame_indices: list[int]) -> np.ndarray | None:
        ...
        for state_key in state_keys:
            state_data[state_key] = self.get_state_or_action(
                trajectory_id,
                "state",
                state_key,
                base_index,
        ...
        transformed = self.transforms(state_data)
        state_parts = []
        for state_key in state_keys:
            value = transformed[state_key]
            if isinstance(value, torch.Tensor):
                value = value.detach().cpu().numpy()
            state_parts.append(np.asarray(value))
        if not state_parts:
            return None
        return np.concatenate(state_parts, axis=1).astype(np.float16)

    def _lookup_astribot_subtask_instruction(self, trajectory_id: int | None, base_index: int) -> str | None:
        if trajectory_id is None or not self._astribot_subtask_by_episode:
            return None
        assert self.curr_traj_data is not None, "No trajectory data loaded"
        if base_index >= len(self.curr_traj_data):
            return None

        index_column = None
        for candidate in ["subtask_instruction_index", "subtask_index"]:
        ...
        value = self.curr_traj_data[index_column].iloc[base_index]
        try:
            subtask_index = int(np.asarray(value).reshape(-1)[0])
        except Exception:
            return None

        episode_meta = self._astribot_subtask_by_episode.get(int(trajectory_id), {})
        instruction_map = episode_meta.get("subtask_instruction_map", {})
        return instruction_map.get(str(subtask_index)) or instruction_map.get(subtask_index)

    def _read_optional_instruction(self, base_index: int, trajectory_id: int | None = None) -> str | None:
        assert self.curr_traj_data is not None, "No trajectory data loaded"
        configured_column = self._history_cfg_get(
            "subtask_instruction_column",
            self._history_cfg_get("instruction_column", None),
        )
        aliases = [
            "subtask_instruction",
            "subtask",
        ...
        try:
            column = self._find_trajectory_column(configured_column, aliases)
        except KeyError:
            return self._lookup_astribot_subtask_instruction(trajectory_id, base_index)

        if base_index >= len(self.curr_traj_data):
            return None
        value = self.curr_traj_data[column].iloc[base_index]
        try:
            if pd.isna(value):
                return None
        except Exception:
        ...
        if isinstance(value, (list, tuple, np.ndarray)):
            ...
            if arr.size == 0:
                return None
            value = arr[0]
        if str(column).endswith("_index"):
            mapped = self._lookup_astribot_subtask_instruction(trajectory_id, base_index)
    ...
    def _pack_sample(self, data: dict, trajectory_id: int | None = None, base_index: int | None = None) -> dict:
        ...
        if self._history_enabled() and trajectory_id is not None and base_index is not None:
            step_images, history_indices = self._pack_history_images(trajectory_id, base_index)
        else:
            step_images = []
            resize_size = self._image_resize_size()
            for video_key in self.modality_keys["video"]:
                image = data[video_key][0]
                image = Image.fromarray(image).resize(resize_size)
        ...
        if history_indices is not None:
            sample["history_frame_indices"] = history_indices
            sample["history_mode"] = self._history_mode()
            sample["num_frames"] = len(step_images)
            sample["num_history_frames"] = max(len(step_images) - 1, 0)
        else:
            sample["num_frames"] = len(step_images)
            sample["num_history_frames"] = 0

        if trajectory_id is not None and base_index is not None:
            subtask_instruction = self._read_optional_instruction(base_index, trajectory_id=trajectory_id)
            if subtask_instruction is not None:
                sample["subtask_lang"] = subtask_instruction
        ...
        if self.data_cfg is not None and self.data_cfg.get("include_state", False) not in ["False", False]:
            ...
            if not state:
                ...
                sample["state"] = state
                if history_indices is not None and trajectory_id is not None and base_index is not None:
                    state_history = self._pack_history_state(trajectory_id, base_index, history_indices)
                    if state_history is not None:
                        sample["state_history"] = state_history
                else:
                    sample["state_history"] = state
